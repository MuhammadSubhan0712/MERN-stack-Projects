...
const RightSidebar = () => {
  ...
  return (
    selectedUser && (
      <>
        <div
          className={`h-full flex flex-col bg-gradient-to-b from-gray-900/80 to-purple-900/20 backdrop-blur-md border-l border-gray-700/30 
   w-3/4 md:w-auto absolute md:static right-0 inset-y-0 z-20 transition-transform duration-300
   ${selectedUser ? "transalate-x-0" : "translate-x-full"}`}>
          <button
            onClick={() => setSelectedUser(null)}
            className="md:hidden p-1.5 rounded-full bg-gray-700/50 hover:bg-gray-700 transition-colors absolute left-4 top-4">
            <FaWindowClose />
          </button>
