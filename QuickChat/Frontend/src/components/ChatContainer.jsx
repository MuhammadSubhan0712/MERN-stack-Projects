...
const ChatContainer = () => {
  ...
  useEffect(() => {
    if (scrollEnd.current) {
      scrollEnd.current.scrollIntoView({
        behavior: "smooth",
        block: "nearest", // to prevent unwanted jumps:
      });
    }
  }, [messages]);
  ...
  return selectedUser ? (
    <div className="h-full flex flex-col bg-gradient-to-b from-gray-900/80 to-purple-900/20 backdrop-blur-md md:static absolute inset-0 z-10">
      ...
      {/* Chat Area */}
      <div
        className="flex-1 overflow-y-auto p-2 md:p-4 space-y-3 bg-gradient-to-b from-gray-900/30 to-transparent
    scrollbar-thin scrollbar-thumb-gray-700">
        {messages.map((msg, index) => (
          <div
            ...
            <div
              className={`max-w-[75%] md:max-w-md relative group active:scale-95 transition-transform ${
                msg.senderId !== authUser._id ? "pl-10" : "pr-10"
              }`}
              onTouchStart={(e) =>
              ...
              <div
                className={`absolute top-0 ${
                  msg.senderId !== authUser._id ? "-left-2" : "-right-2"
                }`}>
                <img
                  src={
                    msg.senderId === authUser._id
